...
function generateCredential(qrCodeURL) {
  ...
  context.fillStyle = packageColor;
  context.fillRect(0, blackY, canvasWidth, blackHeight);

  // Resto do seu código para adicionar as imagens, textos, QR code, etc.
  // ...

  // Salvar o canvas como uma imagem
  const outputFile = `${ingresso}.png`;
  const stream = canvas.createPNGStream();
  const out = fs.createWriteStream(outputFile);
  stream.pipe(out);
  out.on('finish', () => console.log('Imagem gerada com sucesso!'));
...
// Chamar a função de geração da imagem com os parâmetros adequados
generateCredential('#F2B705', 'https://seusite.com');
